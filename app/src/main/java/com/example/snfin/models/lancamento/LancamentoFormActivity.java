...
public class LancamentoFormActivity extends AppCompatActivity {
    ...
    private Integer lancamentoId = -1;

    // 🔥 NOVO: controle da fatura
    private Integer faturaId = null;
    ...
    private void verificarModoEdicao() {
        ...
        // 🔥 captura fatura (vinda do cartão)
        if (intent.hasExtra("faturaId")) {
            int fId = intent.getIntExtra("faturaId", -1);
            if (fId != -1) {
                faturaId = fId;
            }
            Log.d("FORM", "Fatura recebida: " + faturaId);
        }
        ...
        if (intent.hasExtra("id")) {
            ...
            // 🔥 mantém fatura ao editar
            if (intent.hasExtra("fatura")) {
                int fId = intent.getIntExtra("fatura", -1);
                if (fId != -1) {
                    faturaId = fId;
                }
                Log.d("EDIT", "Fatura (edição): " + faturaId);
            }
    ...
    private void salvar() {
        ...
        try {
            ...
            // 🔥 ENVIO SEGURO DA FATURA
            if (faturaId != null) {
                req.setFatura(faturaId);
                Log.d("API", "Fatura enviada: " + faturaId);
            } else {
                Log.d("API", "Sem fatura (modo home)");
            }
